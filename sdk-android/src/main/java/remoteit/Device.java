...
import remoteit.networking.ProServerSocket;
...
import remoteit.remoteit.ConnectD;
import remoteit.remoteit.LoginResponse;
import remoteit.remoteit.RegisteredService;
import remoteit.transport.ProxyTransport;
import remoteit.transport.StreamsTransport;
...
//			connectdArgs.add(String.format("application_type_overload %d", 40));
    ...
    private void handleNewConnection(InputStream inputStream, OutputStream outputStream) {
        try {
            // 1. read first bytes
            byte[] buffer = new byte[remoteit.networking.Helpers.MAX_DATA_LENGTH];
            int readNrOfBytes = inputStream.read(buffer, 0, remoteit.networking.Helpers.MAX_DATA_LENGTH);

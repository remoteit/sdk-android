		...
		*/
		...
		new Thread(() -> {
			try {
				...
				connectdArgs.add(String.format("#"));
...
//			connectdArgs.add(String.format("application_type_overload %d", 40));
	...
	private void handleNewConnection(InputStream inputStream, OutputStream outputStream) {
		try {
			// 1. read first bytes
			byte[] buffer = new byte[com.remoteit.sdk_android.networking.Helpers.MAX_DATA_LENGTH];
			int readNrOfBytes = inputStream.read(buffer, 0, com.remoteit.sdk_android.networking.Helpers.MAX_DATA_LENGTH);
